...
class MDPTrainer:
    ...
    def _generate_initial_hero_batch(self):
        ...
        self.initial_hero_batch = torch.tensor(self.all_hero_combinations, dtype=torch.float)
        ...
        if self.initial_hero_batch.shape[1] != self.agent.hero_tensor_size:
             log.error(f"Generated hero tensor size ({self.initial_hero_batch.shape[1]}) "
    ...
    def _save_metrics(self):
        ...
        column_order = [
            'episode', 'final_epsilon', 'avg_reward', 'cumulative_reward',
            'avg_tile_changes', 'avg_steps_to_threshold', 'n_finished', 'n_total'
        ]
        ...
        try:
            ...
            df.to_csv(self.metrics_csv_path, index=False, float_format='%.4f') # Format floats for readability
    ...
    def visualize_maps(self,
        ...
        for i in indices_to_show:
            ...
            hero_combo_float = hero_combinations[i] # This is the tuple of floats used for tensor

            # Format hero state nicely from the float tuple
            hero_str_parts = []
            hero_str_parts.append(f"{param_names[0]}={int(hero_combo_float[0])}") # Health
            hero_str_parts.append(f"{param_names[1]}={'T' if int(hero_combo_float[1])==1 else 'F'}") # Item1
            hero_str_parts.append(f"{param_names[2]}={'T' if int(hero_combo_float[2])==1 else 'F'}") # Item2
            hero_str_parts.append(f"{param_names[3]}={entry_reverse_map.get(int(hero_combo_float[3]), '?')}") # Entry
            hero_str_parts.append(f"{param_names[4]}={int(hero_combo_float[4])}") # Rooms Left
            ...
            for r in range(height):
                ...
                for c in range(width):
                    tile_val = int(map_tensor[r, c].item()) # Ensure integer for dict lookup
                    color = self.tile_colors.get(tile_val, "default") # Default color if not mapped
                    # Add space for better alignment if numbers are single digit
                    display_char = str(tile_val) # Don't add extra space, let rich handle width
