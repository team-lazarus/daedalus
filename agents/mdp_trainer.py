...
class MDPTrainer:
    ...
    def __init__(
        ...
        self.random_hero_tensors: Optional[torch.Tensor] = None # Shape (N_batch, hero_size), CPU
        self.hero_combinations: List[Tuple] = [] # List of tuples, CPU
        self.batch_size_N: int = 0 # Will be num_initial_maps * heroes_per_map
        self.original_maps: Optional[torch.Tensor] = None # Shape (num_initial_maps, H, W), CPU
        self.map_indices_for_batch: Optional[List[int]] = None # Len N_batch, maps item i to original map index, CPU

        self.tile_colors = {
            0: "grey50", 1: "white", 2: "bright_red", 3: "red",
            4: "dark_red", 5: "magenta", 6: "green",
        }
    ...
    def _generate_initial_maps_random_walk(self):
        ...
        log.info(f"Generating {self.num_initial_maps} initial maps ({width}x{height}) "
        ...
        maps = torch.zeros((self.num_initial_maps, height, width), dtype=torch.long, device=gen_device)
        ...
        for step in range(self.random_walk_steps):
            ...
            actions = torch.randint(0, NUM_ACTIONS, (self.num_initial_maps,), device=gen_device)
    ...
    def _generate_random_hero_tensors(self):
        ...
        """
        log.info(f"Generating {self.heroes_per_map} random hero tensors for each of the {self.num_initial_maps} map templates...")
        ...
        if num_maps == 0:
            ...
            self.batch_size_N = 0
            self.random_hero_tensors = torch.empty((0, self.agent.hero_tensor_size), dtype=torch.float)
        ...
        try:
            ...
            try:
                ...
                entry_map = {entry.value: i for i, entry in enumerate(Entry)} # "top"->0, "right"->1...
                ...
                entry_range = [float(entry_map[e.value]) for e in allowed_entry_enums]
                 ...
                 log.warning("Entry Enum not found or not used in hero_param_ranges. Assuming 'entry' provides numeric values directly.")
                 entry_range = [float(e) for e in self.hero_param_ranges["entry"]] # Assume numeric if Enum fails
            except KeyError as e:
                 log.error(f"Invalid Entry value name provided in hero_param_ranges['entry']: {e}. Expected values like 'top', 'bottom', etc.")
                 raise
            except AttributeError:
                 log.error("hero_param_ranges['entry'] should contain Enum members (like Entry.TOP), not strings, if using Enum.")
                 raise
            ...
            for map_idx in range(num_maps):
                for _ in range(self.heroes_per_map):
                    ...
                    self.map_indices_for_batch.append(map_idx) # Store corresponding map index

            self.random_hero_tensors = torch.tensor(self.hero_combinations, dtype=torch.float) # Stays on CPU for now
            self.batch_size_N = len(self.hero_combinations) # Set actual batch size

            log.info(f"Generated {self.batch_size_N} random hero tensors (Total training batch size)")
    ...
    def _save_metrics(self):
        ...
        column_order = [
            "episode", "final_epsilon", "avg_cumulative_reward", "policy_entropy",
            "avg_steps_to_threshold", "n_finished", "n_total",
        ]
        ...
        except IOError as e: log.error(f"Error saving metrics to {self.metrics_csv_path}: {e}")
        except Exception as e: log.error(f"An unexpected error occurred while saving metrics: {e}")
    ...
    def visualize_maps(
        self,
        original_maps: Optional[torch.Tensor], # The initial set of generated maps (CPU, N_orig, H, W)
        final_maps: Optional[torch.Tensor], # The maps after the last training episode (CPU, N_batch, H, W)
        map_indices_for_batch: Optional[List[int]], # Maps batch item index -> original map index (CPU)
        map_avg_rewards: Dict[int, float], # Original Map Index -> Avg Reward from last episode (CPU data)
        num_to_show: int = 5,
    ):
        """
        ...
        if map_indices_for_batch is None:
            log.warning("Map indices for batch not available, cannot link modified maps.")
        ...
        log.info(f"Visualizing {num_to_show} random map templates (out of {num_maps_total}) with average rewards and final example...")
        ...
        for i, original_map_idx in enumerate(indices_to_show):
            original_map_tensor = original_maps[original_map_idx] # (H, W) tensor on CPU
            avg_reward = map_avg_rewards.get(original_map_idx, float('nan')) # Get reward, default to NaN

            console.print(f"\n=== Map Template {i+1}/{num_to_show} (Original Index: {original_map_idx}) | Avg Reward: {avg_reward:.3f} ===")

            # --- Print Original Map ---
            self._render_map_to_console(original_map_tensor, f"Original Map (Index {original_map_idx})")
            ...
            if final_maps is not None and map_indices_for_batch is not None:
                 if final_maps.shape[0] != len(map_indices_for_batch):
                     log.warning(f"Mismatch between final_maps count ({final_maps.shape[0]}) and map_indices length ({len(map_indices_for_batch)}). Cannot reliably link modified maps.")
                 else:
                    # Find batch indices that correspond to this original map index
                    corresponding_batch_indices = [
                        b_idx for b_idx, m_idx in enumerate(map_indices_for_batch) if m_idx == original_map_idx
                    ]
                    ...
                    if corresponding_batch_indices:
                        ...
                        if 0 <= chosen_batch_idx < final_maps.shape[0]:
                             modified_map_tensor = final_maps[chosen_batch_idx] # (H, W) tensor on CPU
                        else:
                             log.warning(f"Chosen batch index {chosen_batch_idx} out of bounds for final maps.")
            ...
            if modified_map_tensor is not None:
                self._render_map_to_console(modified_map_tensor, f"Example Modified Map (from Batch Item {chosen_batch_idx_str})")
            else:
                 console.print(f"--- Example Modified Map: Not Available (Batch Item: {chosen_batch_idx_str}) ---")
    ...
    def train(self):
        ...
        log.info(f"Modification Threshold: {self.agent.percentage_change_threshold:.1%}")
        ...
        except ValueError as e: # Catch potential size mismatch or other generation errors
        ...
        log.info(f"Total training batch size (Map Templates * Heroes per Map): {self.batch_size_N}")
        ...
        try:
            for batch_idx, map_idx in enumerate(self.map_indices_for_batch):
                if 0 <= map_idx < num_map_templates:
                    ...
                    initial_batch_maps_cpu[batch_idx] = self.original_maps[map_idx].clone()
                else:
                    log.error(f"Invalid map index {map_idx} encountered for batch item {batch_idx}. Check generation logic.")
                    return # Stop if indices are wrong
        except IndexError:
             log.error(f"IndexError during initial batch map creation. Batch size={self.batch_size_N}, map_indices len={len(self.map_indices_for_batch)}, num_templates={num_map_templates}")
             return


        # --- Training Loop Setup ---
        last_episode_map_rewards: Dict[int, float] = {} # Store last rewards for visualization
        last_episode_final_maps: Optional[torch.Tensor] = None # Store last modified maps (on CPU)
        ...
        try:
            with progress:
                ...
                for episode_idx in range(self.num_episodes):
                    ...
                    episode_initial_maps_device = initial_batch_maps_cpu.clone().to(self.device)
                    episode_initial_heroes_device = self.random_hero_tensors.clone().to(self.device) # Heroes are float
                    ...
                    last_episode_final_maps = episode_result.get("final_maps") # Shape (N_batch, H, W), CPU
                    episode_metrics = episode_result.get("metrics", {})
                    final_rewards_tensor = episode_result.get("final_cumulative_rewards") # Shape (N_batch,), CPU
                    ...
                    if final_rewards_tensor is not None and self.map_indices_for_batch is not None and len(final_rewards_tensor) == self.batch_size_N:
                        for i in range(self.batch_size_N):
                            map_idx = self.map_indices_for_batch[i]
                            reward = final_rewards_tensor[i].item() # .item() gets Python number from 0-dim tensor
                            map_rewards_agg[map_idx].append(reward)

                        for map_idx, rewards_list in map_rewards_agg.items():
                             current_map_avg_rewards[map_idx] = sum(rewards_list) / len(rewards_list) if rewards_list else float('nan')
                        last_episode_map_rewards = current_map_avg_rewards # Store for visualization after loop
                    else:
                        log.warning(f"Episode {episode_idx+1}: Could not calculate per-map rewards (final rewards tensor missing, mismatched size, or map indices missing).")
                    ...
                    full_metrics = {
                        ...
                        "final_epsilon": round(episode_metrics.get("final_epsilon", self.agent.epsilon), 5),
                        "avg_cumulative_reward": episode_metrics.get("avg_cumulative_reward", float('nan')),
                        "policy_entropy": episode_metrics.get("policy_entropy", float('nan')),
                        "avg_steps_to_threshold": episode_metrics.get("avg_steps_to_threshold", float('nan')),
