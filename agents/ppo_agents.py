...
class PolicyNetworkEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.linear = nn.Linear(self.cnn_output_dim + self.hero_tensor_size, self.output_size, bias=True)
...
class PPOPolicyNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = PolicyNetworkEncoder(
            channels=channels, input_size=input_size,
            output_size=encoder_output_size, hero_tensor_size=hero_tensor_size
        )
        self.actor_decoder = PolicyNetworkDecoder(
            input_size=encoder_output_size, hidden_sizes=decoder_hidden_sizes,
            output_size=action_dim
        )

    def forward(self, map_tensor: torch.Tensor, hero_tensor: torch.Tensor) -> torch.Tensor:
...
class PPOConfig:
    ...
    n_steps_per_rollout: int = 64 # <<<<< UPDATE FREQUENCY: Update policy every N steps per env
    ...
    def __post_init__(self):
        ...
        if self.minibatch_size > self.batch_size:
            print(f"Warning: Minibatch size ({self.minibatch_size}) > Rollout batch size ({self.batch_size}). Adjusting minibatch size.")
...
def configure_from_yaml(yaml_path: str) -> PPOConfig:
    ...
    try:
        ...
        print(f"Warning: YAML config file not found at {yaml_path}. Using default config.")
        return PPOConfig()
...
def initialize_map_hero(config: PPOConfig) -> Tuple[torch.Tensor, torch.Tensor, Tuple[int, int]]:
    ...
    for _ in range(config.initial_map_walk_steps):
        tile_value = 1 if random.random() < config.initial_map_empty_prob else random.randint(2, 5)
    ...
    if map_tensor[start_pos] == 0: map_tensor[start_pos] = 1 # Ensure start is walkable
...
class BatchedEnvSimulator:
    ...
    def __init__(self, config: PPOConfig, critic_func: Callable):
        ...
        self.maps = torch.zeros((self.num_envs, self.map_size_x, self.map_size_y), dtype=torch.int64, device=self.device)
        self.heroes = torch.zeros((self.num_envs, config.hero_tensor_size), dtype=torch.int64, device=self.device)
        self.agent_positions = [(0, 0)] * self.num_envs # Store agent positions (CPU is fine)
    ...
    def step(self, actions: torch.Tensor) -> Tuple[Tuple[torch.Tensor, torch.Tensor], torch.Tensor, torch.Tensor, Dict]:
        ...
        for i in range(self.num_envs):
            ...
            if mode == "narrow":
                if 0 <= action <= 6: current_map[pos_x, pos_y] = action # 7 is no-op
                ...
                if 0 <= action <= 5: current_map[pos_x, pos_y] = action
                elif action == 6: pos_x = (pos_x - 1 + self.map_size_x) % self.map_size_x # Up
                elif action == 7: pos_y = (pos_y - 1 + self.map_size_y) % self.map_size_y # Left
                elif action == 8: pos_x = (pos_x + 1) % self.map_size_x                 # Down
                elif action == 9: pos_y = (pos_y + 1) % self.map_size_y                 # Right
                ...
                if self.config.action_dim != num_tile_types * n_tiles_total:
                     print(f"Warning: Action dim mismatch for wide mode. Check config. Expected {num_tile_types * n_tiles_total}, got {self.config.action_dim}")
...
class PPOMemory:
    ...
    def __init__(self, num_envs: int, n_steps: int, map_shape: Tuple[int, int], hero_shape: int, device: torch.device):
        self.n_steps, self.num_envs, self.device = n_steps, num_envs, device
        map_c, map_h, map_w = 1, map_shape[0], map_shape[1]
        self.maps = torch.zeros((n_steps, num_envs, map_c, map_h, map_w), dtype=torch.float32, device=device)
        self.heroes = torch.zeros((n_steps, num_envs, hero_shape), dtype=torch.float32, device=device)
        self.actions = torch.zeros((n_steps, num_envs), dtype=torch.int64, device=device)
        self.log_probs = torch.zeros((n_steps, num_envs), dtype=torch.float32, device=device)
        self.rewards = torch.zeros((n_steps, num_envs), dtype=torch.float32, device=device)
        self.dones = torch.zeros((n_steps, num_envs), dtype=torch.bool, device=device)
        self.values = torch.zeros((n_steps, num_envs), dtype=torch.float32, device=device)
        self.advantages: Optional[torch.Tensor] = None
        self.returns: Optional[torch.Tensor] = None
        self.ptr = 0

    def add(self, map_obs: torch.Tensor, hero_obs: torch.Tensor, action: torch.Tensor,
              log_prob: torch.Tensor, reward: torch.Tensor, done: torch.Tensor, value: torch.Tensor):
        """Adds a transition step to the memory."""
        if self.ptr >= self.n_steps: raise IndexError("Memory buffer full")
        self.maps[self.ptr] = map_obs; self.heroes[self.ptr] = hero_obs
        self.actions[self.ptr] = action; self.log_probs[self.ptr] = log_prob
        self.rewards[self.ptr] = reward; self.dones[self.ptr] = done
        self.values[self.ptr] = value
        self.ptr += 1

    def compute_gae_returns(self, last_value: torch.Tensor, gamma: float, gae_lambda: float):
        ...
        for t in reversed(range(self.n_steps)):
            if t == self.n_steps - 1:
                ...
                next_non_terminal = 1.0 # Assume non-terminal unless externally specified
    ...
    def get_minibatches(self, batch_size: int, minibatch_size: int) -> Generator[Dict[str, torch.Tensor], None, None]:
        ...
        if num_transitions == 0: return # No data to process
        if minibatch_size > num_transitions:
             # This can happen if the last rollout in an episode is smaller than minibatch_size
             print(f"Warning: Minibatch size ({minibatch_size}) > available transitions ({num_transitions}). Using {num_transitions} as minibatch size.")
        ...
        valid_maps = self.maps[:self.ptr].reshape(num_transitions, *self.maps.shape[2:])
        ...
        for start_idx in range(0, num_transitions, minibatch_size):
            ...
            yield {
                "maps": valid_maps[mb_indices], "heroes": valid_heroes[mb_indices],
                "actions": valid_actions[mb_indices], "old_log_probs": valid_log_probs[mb_indices],
                "advantages": valid_advantages[mb_indices], "returns": valid_returns[mb_indices],
                "old_values": valid_values[mb_indices], # Add old values for clipped value loss (optional but common)
            }
...
class PPOAgent:
    ...
    def __init__(self, config: PPOConfig):
        ...
        self.actor = PPOPolicyNetwork(
            action_dim=config.action_dim, hero_tensor_size=config.hero_tensor_size,
    ...
    def select_action(self, map_obs: torch.Tensor, hero_obs: torch.Tensor, temperature: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            if temperature > 0: scaled_logits = action_logits / max(temperature, 1e-8)
            else: scaled_logits = action_logits # Deterministic handled below
    ...
    def evaluate_actions(self, map_obs: torch.Tensor, hero_obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def update(self, memory: PPOMemory) -> Dict[str, float]:
        ...
        if memory.ptr == 0: return {} # No data to update from
        ...
        memory.compute_gae_returns(last_value, self.config.gamma, self.config.gae_lambda)
        ...
        for _ in range(self.config.num_epochs_per_update):
            ...
            minibatch_generator = memory.get_minibatches(memory.ptr * self.config.num_envs, self.config.minibatch_size)
            for batch in minibatch_generator:
                mb_maps, mb_heroes, mb_actions = batch["maps"], batch["heroes"], batch["actions"]
                mb_old_log_probs, mb_advantages, mb_returns = batch["old_log_probs"], batch["advantages"], batch["returns"]

                # Normalize advantages at the minibatch level
                mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)

                new_log_probs, new_values, entropy = self.evaluate_actions(mb_maps, mb_heroes, mb_actions)

                # Policy Loss (Clipped Surrogate Objective)
                log_ratio = new_log_probs - mb_old_log_probs
                ratio = torch.exp(log_ratio)
                clip_adv = torch.clamp(ratio, 1.0 - self.config.clip_epsilon, 1.0 + self.config.clip_epsilon) * mb_advantages
                ...
                # Total Loss
                loss = policy_loss - self.config.entropy_coef * entropy_loss + self.config.vf_coef * value_loss

                # Optimization step
                self.optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(list(self.actor.parameters()) + list(self.critic.parameters()), self.config.max_grad_norm)
                self.optimizer.step()
    ...
    def load_checkpoint(self, path: str) -> int:
        ...
        try:
            ...
            if chk_config and (chk_config.action_dim != self.config.action_dim or chk_config.mode != self.config.mode):
                print(f"Warning: Config mismatch (ActionDim/Mode) in checkpoint vs current. Loading weights anyway, but check config compatibility.")
            elif chk_config:
            ...
            start_episode = checkpoint.get('episode', -1) + 1 # Resume from next episode

            print(f"Checkpoint loaded from {path}. Resuming from episode {start_episode} "
                  f"(Total updates in chk: {self.total_updates_performed}, Steps: {self.agent.total_steps_interacted:,}).") # Use agent.total_steps... here
...
def print_map(console: Console, map_tensor: torch.Tensor, title: str = "Generated Map"):
    ...
    if map_tensor.ndim == 3 and map_tensor.shape[0] == 1: map_tensor = map_tensor.squeeze(0)
    if map_tensor.device != torch.device('cpu'): map_tensor = map_tensor.cpu()
    map_np = map_tensor.numpy().astype(int)
    map_size_x, map_size_y = map_np.shape
    colors = {0: "grey50", 1: "white", 2: "bright_red", 3: "red", 4: "dark_red", 5: "red3", 6: "green"}
    default_color, char_width = "magenta", 2
    table = Table(title=title, show_header=False, show_edge=True, box=None, padding=0, expand=False)
    for _ in range(map_size_y): table.add_column(justify="center", width=char_width)
    for r in range(map_size_x):
        row_cells = [f"[{colors.get(tile, default_color)}]{tile:>{char_width-1}}[/]" for tile in map_np[r]] # Adjusted padding
...
class PPOTrainer:
    ...
    def __init__(self, config: PPOConfig, critic_func: Callable):
        ...
        self.memory = PPOMemory(config.num_envs, config.n_steps_per_rollout, config.map_size, config.hero_tensor_size, self.device)
    ...
    def train(self):
        ...
        # --- Setup ---
        self.console.print(Panel.fit(f"Starting PPO Episodic Training: mode='{cfg.mode}', run='{cfg.run_name}'", title="Setup", border_style="blue"))
        self.console.print(f"Device: [cyan]{self.device}[/cyan], Episodes: {cfg.num_episodes}, Steps/Episode: {cfg.episode_length}")
        # Highlight the rollout length which determines update frequency
        self.console.print(f"Envs: {cfg.num_envs}, Steps/Rollout (Update Freq): [bold yellow]{cfg.n_steps_per_rollout}[/], PPO Epochs: {cfg.num_epochs_per_update}, Minibatch: {cfg.minibatch_size}")
        ...
        if os.path.exists(latest_checkpoint_path):
            ...
            if self.start_episode >= cfg.num_episodes:
                 self.console.print(f"[yellow]Checkpoint indicates training already completed ({self.start_episode}/{cfg.num_episodes} episodes). Exiting.[/yellow]")
                 return
        ...
        episode_progress = Progress(
            ...
            console=self.console, transient=False # Keep the bar after completion
        )
        ...
        with episode_progress:
            ...
            for episode in range(self.start_episode, cfg.num_episodes):
                # --- Start of Episode ---
                map_obs, hero_obs, initial_maps_cpu = self.env.reset() # Reset envs, get initial state & maps
                episode_rewards = []
                ...
                if (episode + 1) % cfg.print_maps_freq == 0 or episode == self.start_episode:
                    num_maps_to_print = min(3, cfg.num_envs)
                    # Use the progress bar's console to avoid overlap issues
                    episode_progress.console.print(Panel(f"--- Episode {episode+1}: Initial Maps (First {num_maps_to_print}) ---", expand=False))
                    for i in range(num_maps_to_print):
                        print_map(episode_progress.console, initial_maps_cpu[i], title=f"Ep {episode+1} Initial (Env {i})")
                    episode_progress.console.print("-" * (cfg.map_size[1] * 3 + 4)) # Adjust width based on map size/char width
                ...
                with tqdm(total=cfg.episode_length, desc=f"Ep {episode+1}/{cfg.num_episodes} Steps", leave=False, unit="step", position=0) as step_pbar:
                    while steps_this_episode < cfg.episode_length:
                        ...
                        steps_to_run_this_rollout = min(cfg.n_steps_per_rollout, cfg.episode_length - steps_this_episode)
                        ...
                        for _ in range(steps_to_run_this_rollout):
                            ...
                            self.memory.add(map_obs, hero_obs, action, log_prob, reward, done, value)
                            map_obs, hero_obs = next_map_obs, next_hero_obs # IMPORTANT: Update state

                            # Track rewards and steps
                            episode_rewards.append(reward.mean().item()) # Track avg reward per step
                        ...
                        if self.memory.ptr > 0: # Ensure memory isn't empty before update
                            last_update_metrics = self.agent.update(self.memory) # Store metrics from this update
                ...
                if (episode + 1) % cfg.print_maps_freq == 0 or episode == cfg.num_episodes - 1:
                    ...
                    episode_progress.console.print(Panel(f"--- Episode {episode+1}: Final Maps (First {num_maps_to_print}) ---", expand=False))
                    for i in range(num_maps_to_print):
                        print_map(episode_progress.console, final_maps_cpu[i], title=f"Ep {episode+1} Final (Env {i})")
                    episode_progress.console.print("-" * (cfg.map_size[1] * 3 + 4)) # Adjust width
        ...
        # --- End of Training ---
        self.console.print(Panel(f"Training finished after {cfg.num_episodes} episodes ({self.agent.total_steps_interacted:,} total steps, {self.agent.total_updates_performed:,} updates).", title="Complete", border_style="green"))
        final_chk_path = os.path.join(cfg.checkpoint_dir, "final_checkpoint.pth")
        self.agent.save_checkpoint(final_chk_path, cfg.num_episodes - 1) # Save index of last completed episode
...
if __name__ == "__main__":
    ...
    if os.path.exists(config_path):
         ...
         print(f"Configuration file '{config_path}' not found. Using default settings defined in PPOConfig.")
         config = PPOConfig()
    ...
    try:
        ...
        trainer.agent.save_checkpoint(interrupted_chk_path, latest_episode) # Save with last known good episode index
        print(f"Interrupted state checkpoint saved to {interrupted_chk_path} (based on episode {latest_episode+1})")
    except Exception as e:
        ...
        if os.path.exists(latest_checkpoint_path):
             try:
                 ...
                 print(f"Could not read latest checkpoint episode during error handling: {load_e}")
        ...
        print(f"Error state checkpoint saved to {error_chk_path} (based on episode {latest_episode+1})")
    ...
    try:
        ...
        test_map = torch.rand((2, 1, config.map_size[0], config.map_size[1])).to(config.device)
        ...
        console.print("[green]Network forward pass test successful.[/green]")
         ...
         console.print("[yellow]Agent state/Final checkpoint not available for network test.[/yellow]")
    except Exception as e:
