...
from rich.text import Text  # Import Text for score formatting
from tqdm import tqdm

# --- Local Application/Library Specific Imports ---
# Mock Critic Function (IMPORTANT: Replace with your actual import)
from daedalus.critics import level_critic  # <<< Your actual critic function


actual_critic = level_critic # <<< Use this line when you have the real critic


# --- Model Definitions (Unchanged) ---
class CriticApproximatorCNN(nn.Module):
    """CNN Critic Approximator"""

    def __init__(
        self,
        channels: List[int] = [1, 16, 32, 64],
        ...
        self.conv1 = nn.Conv2d(channels[0], channels[1], kernel_size=3, padding="same")
        self.bn1 = nn.BatchNorm2d(channels[1])
        self.conv2 = nn.Conv2d(channels[1], channels[2], kernel_size=3, padding="same")
        self.bn2 = nn.BatchNorm2d(channels[2])
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(channels[2], channels[3], kernel_size=3, padding="same")
        self.bn3 = nn.BatchNorm2d(channels[3])
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.flattened_size = channels[-1] * (input_size[0] // 4) * (input_size[1] // 4)
        self.fc1 = nn.Linear(self.flattened_size, hidden_fc_size)
        self.fc_out = nn.Linear(hidden_fc_size, output_size)
...
@dataclass
class CriticConfig:
    """Configuration class for Neural Critic training."""
    ...
    cnn_channels: List[int] = field(default_factory=lambda: [1, 16, 32, 64])
    cnn_fc_hidden_size: int = 128
    ...
    # <<< New Config: Control validation map printing >>>
    ...
    def __post_init__(self):
        ...
        if not self.tile_enemy_values:
            if self.tile_enemy_prob > 0:
                print(
                    "Warning: Non-zero enemy probability but no enemy "
                    "tile values specified."
                )
            self.tile_enemy_prob = 0
        model_type = "MLP" if self.use_mlp_critic else "CNN"
        if self.run_name is None:
            self.run_name = f"critic_{model_type}_{time.strftime('%Y%m%d_%H%M%S')}"
...
def configure_critic_from_yaml(yaml_path: str) -> CriticConfig:
    ...
    try:
        ...
        if "use_mlp_critic" in filtered_config:
            filtered_config["use_mlp_critic"] = bool(filtered_config["use_mlp_critic"])
        # Handle new boolean config from YAML
...
def generate_batch(
    ...
    """Generates maps and their 'true' scores from the original critic."""
    maps_batch_cpu = torch.stack(
        [generate_random_map(config) for _ in range(batch_size)]
    )
...
class NeuralCriticTrainer:
    """Orchestrates the training of the neural critic approximator."""
    ...
    def __init__(self, config: CriticConfig, critic_func: Callable):
        ...
        self.model_type = "MLP" if config.use_mlp_critic else "CNN"
        self.console.print(
            f"Initializing [bold blue]{self.model_type}[/] critic model..."
        )
        if config.use_mlp_critic:
            ...
            self.model = CriticApproximatorCNN(
                channels=config.cnn_channels,
                input_size=config.map_size,
                hidden_fc_size=config.cnn_fc_hidden_size,
            ).to(self.device)
            self.console.print(
                f"  CNN Channels: {config.cnn_channels}, "
                f"FC Hidden: {config.cnn_fc_hidden_size}"
            )
        ...
        if os.path.exists(latest_checkpoint_path):
            try:
                checkpoint = torch.load(
                    latest_checkpoint_path, map_location=self.device
                )
                chk_config = checkpoint.get("config")
                if (
                    chk_config
                    and hasattr(chk_config, "use_mlp_critic")
                    and chk_config.use_mlp_critic != self.config.use_mlp_critic
                ):
                    raise ValueError(
                        "Model type mismatch between loaded checkpoint and current config."
                    )
                ...
                self.console.print(
                    f"Checkpoint loaded from [green]{latest_checkpoint_path}[/green]. Resuming from epoch {self.start_epoch}."
                )
            except Exception as e:
                self.console.print(
                    f"[red]Error loading checkpoint: {e}. Starting from scratch.[/red]"
                )
                self.start_epoch = 0
                self.total_steps_trained = 0
        else:
            self.console.print("No checkpoint found. Starting training from scratch.")
            self.start_epoch = 0
            self.total_steps_trained = 0
    ...
    def _print_validation_example(
        ...
        """Prints a single validation map example with scores."""
        ...
        colors = {
            0: "dim grey50",  # Dim gray for 0
            1: "white",
            6: "bright_green",  # Bright green for 6
            # Add specific reds for 2-5 if needed, otherwise one red
            2: "bright_red",
            3: "red",
            4: "dark_red",
            5: "red3",
        }
        default_color = "magenta"  # For any unexpected tile values
        char_width = 2  # Width for each tile cell
        ...
        for r in range(map_size_x):
            ...
            for tile in map_np[r]:
                ...
                # Right-align tile value within the cell width
                row_cells.append(
                    f"[{color}]{tile:>{char_width-1}} [/]"
        ...
        self.console.print("-" * (map_size_y * (char_width + 1) + 1))  # Separator line

    def _validate(self) -> float:
        """Performs validation and prints example maps if configured."""
        self.model.eval()
        total_val_loss = 0.0
        ...
        # Store data from the *last* batch for printing examples
        last_batch_maps = None
        last_batch_true_scores = None
        last_batch_pred_scores = None

        with torch.no_grad():
            for i in range(num_batches):
                val_maps, val_true_scores = generate_batch(
                    self.config.batch_size,
                    self.config,
                    self.critic_func,
                    self.device,
                )
                val_pred_scores = self.model(val_maps)
                val_loss = self.criterion(val_pred_scores, val_true_scores)
                total_val_loss += val_loss.item()

                # If this is the last batch, store its data
                if i == num_batches - 1:
                    # We need the original map data *before* unsqueeze/float
                    # Regenerate maps for the last batch on CPU to print original ints
                    # This avoids storing potentially large tensors on GPU during loop
                    last_batch_maps_cpu = torch.stack(
                        [
                            generate_random_map(self.config)
                            for _ in range(val_maps.size(0))
                        ]  # Use actual batch size
                    )
                    last_batch_maps = (
                        last_batch_maps_cpu  # Store the IntTensor (B, H, W)
                    )
                    last_batch_true_scores = val_true_scores.squeeze(-1).cpu()  # (B,)
                    last_batch_pred_scores = val_pred_scores.squeeze(-1).cpu()  # (B,)

        avg_val_loss = total_val_loss / num_batches
        self.model.train()  # Set back to training mode

        # --- Print Validation Map Examples ---
        if (
            self.config.print_validation_maps
            and last_batch_maps is not None
            ...
            num_examples = min(
                ...
                last_batch_maps.shape[0],  # Don't try to print more than batch size
            )
            if num_examples > 0:
                self.console.print(
                    Panel(
                ...
                indices = random.sample(range(last_batch_maps.shape[0]), num_examples)
                for i, idx in enumerate(indices):
                    map_to_print = last_batch_maps[idx]  # Shape (H, W)
    ...
    def train(self):
        ...
        # (Train loop logic mostly unchanged, but _validate now handles printing)
        cfg = self.config
        self.console.print(
            Panel.fit(
                f"Starting Neural Critic Training: run='{cfg.run_name}', model='{self.model_type}'",
                title="Setup",
                border_style="blue",
            )
        )
        ...
        with progress:
            task = progress.add_task(
                f"[cyan]Training Critic ({self.model_type})",
                total=total_training_steps,
                completed=self.total_steps_trained,
                metrics=initial_metrics,
            )
            current_step_in_epoch = self.total_steps_trained % cfg.steps_per_epoch
            ...
            while self.total_steps_trained < total_training_steps:
                ...
                maps, true_scores = generate_batch(
                    cfg.batch_size, cfg, self.critic_func, self.device
                )
                predicted_scores = self.model(maps)
                ...
                current_step_in_epoch = self.total_steps_trained % cfg.steps_per_epoch
                if current_step_in_epoch == 0:
                    current_step_in_epoch = cfg.steps_per_epoch
                train_mse = loss.item()
                ...
                try:
                    ...
                    last_val_mse_str = "----    "
                metrics_str = f" Train MSE:[red]{train_mse:>8.4f}[/]| Val MSE:{last_val_mse_str}"

                if (
                    self.total_steps_trained > 0
                    and self.total_steps_trained % cfg.validation_freq == 0
                ):
                    # _validate now handles printing internally if configured
                ...
                # --- Checkpointing ---
                if current_step_in_epoch == cfg.steps_per_epoch:
        ...
        self.console.print(
            Panel(
                f"Training finished after {cfg.num_epochs} epochs ({self.total_steps_trained:,} total steps). Model: {self.model_type}",
                title="Complete",
                border_style="green",
            )
        )
        ...
        final_chk_path = os.path.join(
            cfg.checkpoint_dir, f"final_checkpoint_epoch_{cfg.num_epochs}.pth"
        )
...
if __name__ == "__main__":
    ...
    # --- Optional: Test the trained model (Unchanged) ---
    console = Console()
    print("\n--- Optional: Testing Trained Critic Approximator ---")
    try:
        test_config = trainer.config
        test_device = trainer.device
        model_type_used = "MLP" if test_config.use_mlp_critic else "CNN"
        console.print(
            f"Attempting to test the trained [bold blue]{model_type_used}[/] model..."
        )
        if test_config.use_mlp_critic:
            ...
            test_model = CriticApproximatorCNN(
                channels=test_config.cnn_channels,
                input_size=test_config.map_size,
                hidden_fc_size=test_config.cnn_fc_hidden_size,
            ).to(test_device)
        latest_checkpoint_path = os.path.join(
            test_config.checkpoint_dir, "latest_checkpoint.pth"
        )
        if os.path.exists(latest_checkpoint_path):
            checkpoint = torch.load(latest_checkpoint_path, map_location=test_device)
            chk_cfg = checkpoint.get("config")
            assert not (
                chk_cfg
                and hasattr(chk_cfg, "use_mlp_critic")
                and chk_cfg.use_mlp_critic != test_config.use_mlp_critic
            ), "Model type mismatch in test"
            ...
            table = Table(title=f"Critic Approximation Test ({model_type_used})")
