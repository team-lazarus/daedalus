    ...
    while queue:
        ...
        for ny, nx in get_neighbors(cy, cx, height, width):
            if ((ny, nx) not in visited and 
                is_traversable(map_tensor[ny, nx].item())):
                queue.append((ny, nx))
                visited.add((ny, nx))
...
def find_contiguous_areas(map_tensor: torch.Tensor) -> List[Set[Tuple[int, int]]]:
    ...
    if height == 6 and width == 6:
        ...
        if center_area_count == 16:
            ...
            for y in range(1, 5):
                for x in range(1, 5):
                    if ((y, x) not in visited and 
                        is_traversable(map_tensor[y, x].item())):
...
def count_edge_doors(door_positions: List[Tuple[int, int]], height: int, width: int) -> int:
    """
...
def count_enemies(map_tensor: torch.Tensor) -> int:
    ...
    """
    return count_by_condition(map_tensor, 
                             lambda tile: ENEMY_MIN <= tile <= ENEMY_MAX)
...
def count_traversable_tiles(map_tensor: torch.Tensor) -> int:
    ...
    """
    return count_by_condition(map_tensor, 
                             lambda tile: is_traversable(tile))


def evaluate_contiguity(contiguous_areas: List[Set[Tuple[int, int]]]) -> Tuple[float, int]:
    """
...
def evaluate_enemy_count(
    ...
    if traversable_tiles > 0:
        ...
        if enemy_ratio >= desired_ratio:
            ...
            excess_enemies = max(0, enemy_count - int(traversable_tiles * desired_ratio))
            if excess_enemies > 0:
...
def evaluate_traversable_tiles(map_tensor: torch.Tensor) -> float:
    ...
    """
    traversable_count = count_by_condition(map_tensor, 
                                          lambda tile: 1 <= tile <= 6)
...
def level_critic(map_batch: torch.Tensor, hero_batch: torch.Tensor) -> torch.Tensor:
    ...
    for i in range(n_batch):
        ...
        rewards[i] += evaluate_disconnected_tiles(
            primary_area_size, traversable_tiles)
        ...
        rewards[i] += evaluate_doors(
            door_positions, primary_area, height, width, entry)
        
        # Evaluate enemy distribution
        enemy_count = count_enemies(current_map)
        rewards[i] += evaluate_enemy_count(
            enemy_count, traversable_tiles, health)
...
def create_test_hero(health=8.0, entry_idx=1) -> torch.Tensor:
    ...
    hero = torch.tensor([health, 1., 0., float(entry_idx), 5.])
    return hero
...
if __name__ == "__main__":
    ...
    print(f"Size of primary area: {len(contiguous_areas[0]) if contiguous_areas else 0}")
