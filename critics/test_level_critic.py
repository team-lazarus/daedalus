...
from .level_critic import (
    level_critic, Entry, WALL, EMPTY_MIN, EMPTY_MAX, DOOR, ENEMY_MIN, ENEMY_MAX,
    evaluate_contiguity, evaluate_disconnected_tiles, evaluate_doors,
    evaluate_enemy_count, find_contiguous_areas, count_enemies,
    count_traversable_tiles, find_door_positions, evaluate_traversable_tiles
)
...
def test_traversable_tiles_reward(sample_map):
    ...
    traversable_count = sum(1 for y in range(5) for x in range(7) 
                         if 1 <= sample_map[y, x] <= 6)
...
def test_disconnected_tiles_penalty():
    ...
    disconnected_penalty = evaluate_disconnected_tiles(primary_area_size, total_traversable)
...
def test_door_wrong_entry():
    ...
    hero = torch.tensor([8., 1., 0., 1., 5.])  # Entry.RIGHT
...
def test_level_critic_complete():
    ...
    # Create heroes
    hero1 = torch.tensor([8., 1., 0., 1., 5.])  # RIGHT entry
    hero2 = torch.tensor([2., 1., 0., 1., 5.])  # RIGHT entry, low health
