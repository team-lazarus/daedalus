...
class MDPAgent:
    ...
    def __init__(
        ...
        # Action Space Sizes
        self.num_tile_place_values = (
            NUM_TILE_VALUES - 1
        )  # Values 1 through 6 (SET_EMPTY to PLACE_DOOR)

        if self.strategy == "turtle":
            self.num_actions_turtle_change = NUM_TILE_VALUES  # Includes NO_ACTION(0)
            self.total_turtle_actions = (
                NUM_TURTLE_MOVES + self.num_actions_turtle_change
            )  # Moves + Change Tile (incl NO_ACTION)
            ...
            self.total_wide_actions = (
                self.map_width * self.map_height * self.num_tile_place_values
            )
            ...
            self.num_actions_narrow = (
                NUM_TILE_VALUES  # Includes NO_ACTION(0) for changing the current tile
            )
        ...
        self.config = {
            ...
            if k not in ["self", "critic", "__class__"]
        }
    ...
    def _get_greedy_action(
        ...
        with torch.no_grad():
            ...
            if self.strategy == "narrow":
                assert (
                    current_coords_narrow is not None
                ), "Narrow strategy requires current_coords_narrow"
                x, y = current_coords_narrow
                num_sim_actions = (
                    self.num_actions_narrow - 1
                )  # Actions 1 through 6 (tile changes)
                ...
                simulated_maps = current_map_batch.unsqueeze(1).repeat(
                    1, num_sim_actions, 1, 1
                )
                action_values = torch.arange(
                    1, self.num_actions_narrow, device=self.device
                )  # Values 1 to 6
                ...
                simulated_maps_flat = simulated_maps.view(
                    -1, self.map_width, self.map_height
                )
                simulated_heroes = current_hero_batch.repeat_interleave(
                    num_sim_actions, dim=0
                )

                # Get rewards for simulated actions
                rewards = self.critic(simulated_maps_flat, simulated_heroes).view(
                    n_batch, num_sim_actions
                )
                ...
                best_actions = torch.where(
                    take_best_sim_action,
                    best_sim_action_indices + 1,
                    torch.tensor(NO_ACTION, device=self.device),
                )

            elif self.strategy == "turtle":
                assert (
                    agent_pos_batch is not None
                ), "Turtle strategy requires agent_pos_batch"
                # Evaluate tile change actions (1-6) at the current agent position
                num_sim_change_actions = (
                    self.num_actions_turtle_change - 1
                )  # Actions 1 through 6
                if num_sim_change_actions > 0:
                    ...
                    simulated_change_maps = current_map_batch.unsqueeze(1).repeat(
                        1, num_sim_change_actions, 1, 1
                    )
                    action_values = torch.arange(
                        1, self.num_actions_turtle_change, device=self.device
                    )  # 1 to 6
                    # Apply changes at current agent positions for each simulation
                    simulated_change_maps[
                        batch_indices.unsqueeze(1),
                        torch.arange(num_sim_change_actions).unsqueeze(0),
                        x_coords.unsqueeze(1),
                        y_coords.unsqueeze(1),
                    ] = action_values.unsqueeze(0)

                    simulated_change_maps_flat = simulated_change_maps.view(
                        -1, self.map_width, self.map_height
                    )
                    simulated_heroes_change = current_hero_batch.repeat_interleave(
                        num_sim_change_actions, dim=0
                    )
                    change_rewards = self.critic(
                        simulated_change_maps_flat, simulated_heroes_change
                    ).view(n_batch, num_sim_change_actions)
                else:
                    change_rewards = torch.empty(
                        (n_batch, 0), device=self.device
                    )  # No change actions possible
                ...
                no_change_reward_val = (
                    NUM_TURTLE_MOVES  # Action index for NO_ACTION tile change is 4
                )
                no_change_reward = baseline_reward.unsqueeze(
                    1
                )  # Action corresponding to value NO_ACTION(0)

                all_rewards = torch.cat(
                    [move_rewards, no_change_reward, change_rewards], dim=1
                )
                ...
                sampled_action_indices = torch.randint(
                    0, self.total_wide_actions, (n_batch, k), device=self.device
                )

                # Decode flat actions: tile value (1-6), x, y
                action_val_indices = (
                    sampled_action_indices % self.num_tile_place_values
                )  # 0 to num_tile_place_values-1
                tile_values = action_val_indices + 1  # 1 to 6 (actual tile value)
                flat_coords_times_vals = (
                    sampled_action_indices // self.num_tile_place_values
                )
                ...
                simulated_maps = (
                    current_map_batch.unsqueeze(1).expand(-1, k, -1, -1).clone()
                )
                simulated_heroes = current_hero_batch.repeat_interleave(k, dim=0)

                batch_idx_rep = (
                    torch.arange(n_batch, device=self.device).unsqueeze(1).expand(-1, k)
                )
                simulated_maps[
                    batch_idx_rep,
                ...
                rewards = self.critic(simulated_maps_flat, simulated_heroes).view(
                    n_batch, k
                )  # Reshape rewards back

                best_sampled_rewards, best_indices_in_k = torch.max(rewards, dim=1)
                best_sampled_flat_actions = sampled_action_indices.gather(
                    1, best_indices_in_k.unsqueeze(1)
                ).squeeze(1)
                ...
                best_actions = torch.where(
                    take_best_sampled_action,
                    best_sampled_flat_actions + 1,
    ...
    def _select_action(
        ...
        )  # Single random draw for exploration decision
        ...
        if explore:
            ...
            if total_actions > 0:
                ...
                chosen_actions = torch.randint(
                    0, total_actions, (n_batch,), device=self.device
                )
            else:  # No actions possible at all
                chosen_actions = torch.zeros(
                    n_batch, dtype=torch.long, device=self.device
                )  # Default to NO_ACTION/0
    ...
    def _apply_action(
        ...
        if self.strategy == "narrow":
            ...
            attempt_mask = (
                action_values > NO_ACTION
            )  # Mask for actions that intend to change the tile (value > 0)
            ...
            assert (
                next_agent_pos_batch is not None
            ), "Turtle strategy requires agent_pos_batch"
            current_x, current_y = (
            ...
            if torch.any(move_mask):
                ...
                pos_to_update = next_agent_pos_batch[
                    move_mask
                ].clone()  # Clone slice to avoid modifying original inplace during calculation
                ...
                next_agent_pos_batch[batch_idx_move] = (
                    pos_to_update  # Update the main tensor
                )
            ...
            if torch.any(change_action_mask):
                tile_values_intended = (
                    action_batch[change_action_mask] - NUM_TURTLE_MOVES
                )  # Map actions 4.. to values 0..
                ...
                if torch.any(attempt_mask):
                    ...
                    # Apply the change
                    next_map_batch[batch_idx_attempt, x_attempt, y_attempt] = (
                        action_val_attempt
                    )
                    ...
                    if batch_idx_effective.numel() > 0:
                        ...
                        next_modified_mask_batch[
                            batch_idx_effective, x_effective, y_effective
                        ] = True
            ...
            valid_action_mask = (
                action_batch > 0
            )  # Ignore if action is 0 (no-op chosen by greedy or random)
            if torch.any(valid_action_mask):
                action_batch_valid = (
                    action_batch[valid_action_mask] - 1
                )  # Adjust back to 0-based flat index
                batch_indices_valid = batch_indices[valid_action_mask]

                # Decode flat actions
                action_val_indices = (
                    action_batch_valid % self.num_tile_place_values
                )  # 0 to num_tile_place_values-1
                ...
                # Apply the change
                next_map_batch[batch_indices_valid, target_x, target_y] = (
                    tile_values_intended
                )
                ...
                if batch_idx_effective.numel() > 0:
                    next_modified_mask_batch[
                        batch_idx_effective, target_x_effective, target_y_effective
                    ] = True

        else:
            raise NotImplementedError("Unknown strategy in _apply_action")
    ...
    def _check_initial_tiles_placed(self, map_batch: torch.Tensor) -> torch.Tensor:
        ...
        tile_values_to_check = torch.arange(
            1, NUM_TILE_VALUES, device=self.device
        )  # 1 to 6
        num_tiles_to_check = len(tile_values_to_check)

        map_flat = map_batch.view(n_batch, -1)
        present_mask = (
            map_flat.unsqueeze(1) == tile_values_to_check.view(1, -1, 1)
        ).any(dim=2)
        all_placed_mask = present_mask.all(
            dim=1
        )  # (n_batch,) - True if all tiles 1-6 exist
    ...
    def train_episode(
        ...
        initial_tiles_placed = self._check_initial_tiles_placed(
            current_map_batch
        )  # Check initial state
        ...
        if self.strategy == "turtle":
            ...
            for i in range(total_map_tiles):
                ...
                current_x_offset = (
                    start_x + (start_y + i) // self.map_height
                ) % self.map_width
        ...
        for step in pbar:
            ...
            actions = self._select_action(
                active_maps, active_heroes, active_agent_pos, current_narrow_coords
            )
            ...
            if torch.any(can_terminate_mask):
                ...
                current_mod_counts = modified_mask_batch[eligible_indices].sum(
                    dim=(1, 2)
                )
                terminated_now_eligible_mask = (
                    current_mod_counts >= self.max_modifications
                )

                terminated_now_global_mask = torch.zeros_like(active_mask)
                terminated_now_global_mask[
                    eligible_indices[terminated_now_eligible_mask]
                ] = True
            ...
            if num_active > 0:
                avg_cum_rew_active = (
                    episode_cumulative_reward[active_mask].mean().item()
                )
        ...
        batch_avg_cumulative_reward = (
            episode_cumulative_reward.mean().item() if n_batch > 0 else 0.0
        )

        finished_mask = (steps_to_threshold != -1) & (
            steps_to_threshold <= actual_max_steps
        )
        if torch.any(finished_mask):
            batch_avg_steps_to_threshold = (
                steps_to_threshold[finished_mask].float().mean().item()
            )
        else:
            batch_avg_steps_to_threshold = float(
                "nan"
            )  # No agents finished within steps
    ...
    def load_state(self, filepath: str):
        ...
        try:
            ...
            log.info(
                f"Agent state loaded from {filepath}. Epsilon set to {self.epsilon:.4f}"
            )

        except FileNotFoundError:
            log.error(f"State file not found: {filepath}")
        except (IOError, json.JSONDecodeError) as e:
            log.error(f"Error reading state file {filepath}: {e}")
        except Exception as e:
