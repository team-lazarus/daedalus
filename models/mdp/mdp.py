...
from rich.progress import Progress, SpinnerColumn, BarColumn, TextColumn, TimeElapsedColumn, TimeRemainingColumn # For better episode progress
...
class MDPAgent:
    ...
    def __init__(self,
        ...
        self.max_modifications = math.ceil(self.map_width * self.map_height * percentage_change)
        ...
        log.info(f"MDPAgent initialized on device: {self.device}. Strategy: {self.strategy}")
        ...
        if self.strategy == "turtle":
            ...
            self.total_turtle_actions = self.num_actions_turtle_move + self.num_actions_turtle_change
        elif self.strategy == "wide":
            self.num_wide_change_actions = self.num_tile_change_actions - 1 # Exclude NO_ACTION equivalent
            self.total_wide_actions = self.map_width * self.map_height * self.num_wide_change_actions
        else: # narrow
    ...
    def _get_greedy_action(self,
        ...
        with torch.no_grad():
            ...
            if self.strategy == "narrow":
                ...
                for action_idx in range(1, self.num_actions_narrow): # Skip NO_ACTION (0)
                    next_map_batch = current_map_batch.clone()
                    next_map_batch[:, x, y] = action_idx
                    possible_next_maps.append(next_map_batch)

                if not possible_next_maps: # If only NO_ACTION is possible (shouldn't happen here)
                    return torch.zeros(n_batch, dtype=torch.long, device=self.device) # Return NO_ACTION

                # Evaluate simulated maps
                simulated_maps = torch.stack(possible_next_maps, dim=1).view(-1, self.map_width, self.map_height)
                # Repeat hero batch to match map simulations
                simulated_heroes = current_hero_batch.repeat_interleave(len(possible_next_maps), dim=0)

                rewards = self.critic(simulated_maps, simulated_heroes).view(n_batch, -1)
                ...
                best_sim_rewards = torch.gather(rewards, 1, best_sim_action_indices.unsqueeze(1)).squeeze(1)
                ...
                best_actions = torch.where(take_best_sim_action, best_sim_action_indices + 1, NO_ACTION)

            # Turtle Strategy
            elif self.strategy == "turtle":
                assert agent_pos_batch is not None
                possible_rewards = []
                current_state_reward = self.critic(current_map_batch, current_hero_batch)
                ...
                for change_action_idx in range(1, self.num_actions_turtle_change): # Start from 1 (SET_EMPTY)
                    next_map_batch = current_map_batch.clone()
                    # Apply change action to current agent positions
                    next_map_batch[batch_indices, x_coords, y_coords] = change_action_idx
                    possible_rewards.append(self.critic(next_map_batch, current_hero_batch))

                # Stack all rewards (moves + no_change + changes)
                all_rewards = torch.stack(possible_rewards, dim=1) # Shape: (n_batch, total_turtle_actions)
            ...
            # Wide Strategy
                ...
                best_overall_actions = torch.zeros(n_batch, dtype=torch.long, device=self.device)
                ...
                for change_action_idx in range(1, self.num_tile_change_actions):
                    ...
                    for x in range(self.map_width):
                        for y in range(self.map_height):
                            ...
                            best_overall_actions = torch.where(is_better, current_flat_action, best_overall_actions)
    ...
    def _select_action(self,
        ...
        greedy_actions = self._get_greedy_action(current_map_batch, current_hero_batch, agent_pos_batch, current_coords)
    ...
    def _apply_action(self,
        ...
        if self.strategy == "narrow":
            assert current_coords is not None; x, y = current_coords
            ...
            if torch.any(change_mask):
                ...
                if torch.any(is_actual_change):
                    ...
                    batch_idx_actual_change = batch_indices[change_mask][is_actual_change]
                    ...
                    action_val_change = tile_values[is_actual_change] # Use the non-zero tile values

                    # Apply the changes to the map and modification mask
                    next_map_batch[batch_idx_actual_change, x_change, y_change] = action_val_change
                    next_modified_mask_batch[batch_idx_actual_change, x_change, y_change] = True
             ...
             tile_values = action_val_indices + 1 # Map to SET_EMPTY, PLACE_ENEMY_1, ... PLACE_DOOR
        ...
        return next_map_batch, next_modified_mask_batch, next_agent_pos_batch, tile_changed_mask
    ...
    def train_episode(self,
        ...
        log.debug(f"Starting episode for {n_batch} instances. Max steps: {max_steps}. Strategy: {self.strategy}")

        # --- Initialization for Episode ---
        current_map_batch = initial_map_state.clone().to(self.device)
        current_hero_batch = initial_hero_state.clone().to(self.device) # Keep hero state constant for the episode
        ...
        active_mask = torch.ones(n_batch, dtype=torch.bool, device=self.device) # Tracks which batch items are still running

        agent_pos_batch = None
        narrow_x, narrow_y = 0, 0
        if self.strategy == "turtle":
            # Start turtle at (0,0) for all batch items
            agent_pos_batch = torch.zeros((n_batch, 2), dtype=torch.long, device=self.device)
        ...
        episode_tile_change_count = torch.zeros(n_batch, dtype=torch.long, device=self.device)
        ...
        pbar = tqdm(range(max_steps), desc=f"Ep Step (ε={self.epsilon:.3f})", leave=False, dynamic_ncols=True, ascii=True)
        for step in pbar:
            ...
            if len(active_indices) == 0: # Should be caught by the check above, but safety first
                 break

            active_maps = current_map_batch[active_mask]
            active_heroes = current_hero_batch[active_mask] # Use the corresponding hero states
            ...
            current_narrow_coords = (narrow_x, narrow_y) if self.strategy == "narrow" else None
            ...
            # --- Action Application ---
            next_maps, next_mod_masks, next_agent_pos, tile_changed = self._apply_action(
                active_maps,
                actions,
                active_mod_masks,
                active_agent_pos,
            ...
            steps_to_threshold[record_step_mask] = step + 1 # Record the step number (1-based)
            ...
            if self.strategy == "narrow":
                ...
                if narrow_y >= self.map_height:
                    ...
                    if narrow_x >= self.map_width:
                        ...
                        steps_to_threshold[(steps_to_threshold == -1) & active_mask] = step + 1
            ...
            if torch.any(active_mask):
                 avg_cum_reward_active = torch.mean(episode_cumulative_reward[active_mask]).item()
                 pbar.set_description(f"Ep Step (Act:{torch.sum(active_mask).item()}/{n_batch}, ε={self.epsilon:.3f})")
        ...
        avg_reward_per_step[valid_steps_mask] = episode_cumulative_reward[valid_steps_mask] / episode_steps_taken[valid_steps_mask].float()
        ...
        batch_avg_tile_changes = torch.mean(episode_tile_change_count.float()).item() # Entropy proxy

        # Calculate average steps to threshold for those that finished
        finished_mask = steps_to_threshold != -1
        if torch.any(finished_mask):
             batch_avg_steps_to_threshold = torch.mean(steps_to_threshold[finished_mask].float()).item()
        else:
             # Handle case where no items finished (e.g., max_steps too low)
             batch_avg_steps_to_threshold = float('nan') # Or max_steps, depending on desired representation
    ...
    def load_state(self, filepath: str):
        ...
        try:
            ...
            log.info(f"Agent state loaded successfully from {filepath}. Current epsilon: {self.epsilon:.4f}")
