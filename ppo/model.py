...
class PolicyNetworkEncoder(nn.Module):
    def __init__(
        ...
        downsampled_x = (
            self.input_size_x // 2 // 2
        )  # Two downsampling layers with stride 2
        downsampled_y = self.input_size_y // 2 // 2
        self.cnn_output = (
            downsampled_x * downsampled_y * channels[-1]
        )  # Use last channel count
        ...
        self.linear = nn.Linear(
            self.cnn_output + self.hero_tensor_size, self.output_size
        )
...
class ActorCritic(nn.Module):
    def __init__(self, config: "TrainConfig"):  # Use forward reference for type hint
        super().__init__()
