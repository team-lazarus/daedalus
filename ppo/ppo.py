...
def compute_advantages_gae(rewards: torch.Tensor, values: torch.Tensor, dones: torch.Tensor, gamma: float, gae_lambda: float) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if values.shape[0] != rewards.shape[0] + 1:
        raise ValueError(f"Values tensor shape mismatch. Expected {rewards.shape[0]+1}, got {values.shape[0]}")
    ...
    for step in reversed(range(num_steps)):
        ...
        advantages[step] = last_gae_lam = delta + gamma * gae_lambda * mask * last_gae_lam
...
def ppo_update(agent: 'PPOAgent', batch: Dict[str, torch.Tensor], config: TrainConfig, device: torch.device):
    ...
    dataset = TensorDataset(maps, heroes, actions, log_probs_old, advantages, value_targets)
    # PPO typically iterates over the same batch multiple times
    loader = DataLoader(dataset, batch_size=config.batch_size // config.ppo_epochs, shuffle=True) # Smaller batches for inner epochs
    ...
    for _ in range(config.ppo_epochs):
        for map_b, hero_b, act_b, logp_old_b, adv_b, v_targ_b in loader:
            ...
            dist_new = agent.actor_critic.get_action_distribution(logits_new, temperature=1.0) # Use temp=1 for loss calcs
