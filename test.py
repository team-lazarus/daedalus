...
WALKABLE_CONTIGUOUS_TILES = [
    FLOOR_TILE,
    ENEMY_1,
    ENEMY_2,
    ENEMY_3,
    ENEMY_4,
]  # Tiles considered for contiguity check
...
def _render_map_to_console(map_tensor: torch.Tensor, title: str):
    ...
    if map_tensor.dim() != 2:
        console.print(
            f"[red]Error: Map tensor must be 2D, but got shape {map_tensor.shape}[/red]"
        )
        return
...
def _check_contiguity(map_tensor: torch.Tensor) -> Tuple[bool, int, int]:
                    ...
                    while q:
                        ...
                        for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                            ...
                            if 0 <= nr < height and 0 <= nc < width:
                                ...
                                if (
                                    neighbor_tile in WALKABLE_CONTIGUOUS_TILES
    ...
    is_fully_contiguous = (num_major_components == 1) and (
        largest_component_size == total_walkable_tiles
    )
...
class MapGenEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = gym.spaces.Discrete(
            self.map_height * self.map_width * NUM_PLACEABLE_TILES
        )
    ...
    def _parse_hero_ranges(self):
        ...
        self.entry_range_indices = [
            float(self.entry_map[e.value]) for e in self.allowed_entry_enums
        ]
    ...
    def _generate_initial_map_random_walk(self, steps=25):
        ...
        for _ in range(steps):
            ...
            if (
                map_tensor[pos_y, pos_x] == EMPTY_TILE
            ):  # Only place floor on empty tiles
    ...
    def _normalize_observation(
        self, map_tensor: torch.Tensor, hero_vector: torch.Tensor
    ) -> np.ndarray:
        ...
        for i, feat in enumerate(self.hero_feature_order):
            ...
            if max_val > min_val:
                ...
                hero_normalized[i] = (
                    0.0 if min_val == 0.0 else 0.5
                )  # Or map to 0/1 directly
    ...
    def _calculate_reward(self) -> float:
        ...
        walkable_tiles_count = self._count_tiles(
            map_tensor, WALKABLE_CONTIGUOUS_TILES
        )  # Count 1-5
        ...
        try:
            ...
            hero_entry_direction = next(
                e for e, idx in self.entry_map.items() if idx == hero_entry_idx
            )
        ...
        door_locations = (map_tensor == DOOR).nonzero(
            as_tuple=False
        )  # Get (y, x) pairs
        ...
        if hero_entry_direction:
            for loc in door_locations:
                ...
                if hero_entry_direction == Entry.TOP.value and y == 0:
                    total_reward += 1.0
                    entry_door_reward_applied = True
                    break
        ...
        num_floor_tiles = self._count_tiles(
            map_tensor, [FLOOR_TILE]
        )  # Ratio based on tile 1 (floor)
        ...
        if num_floor_tiles > 0:
            ...
            if hero_health < 3.0:  # Using the exact value from the description
                ...
                extra_enemies_low_health = max(
                    0, num_enemies - desired_max_enemies_low_health
                )
        ...
        if num_doors > 4:
            ...
            for loc in door_locations:
                ...
                if not is_on_edge:
                    door_penalty = (
                        -2.0
                    )  # Penalty applies once if any door is not on edge
                    break
    ...
    def reset(self, seed=None, options=None):
        ...
        if (
            ...
            and MapGenEnv.episode_counter % RENDER_EVERY_N_EPISODES == 0
        ):
            if MapGenEnv.episode_counter != self.last_map_rendered_episode:
                self.render()
                self.last_map_rendered_episode = (
                    MapGenEnv.episode_counter
                )  # Avoid double render if reset called mid-episode
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Max steps per episode (map modifications)",
    )
    parser.add_argument(
        "--hid",
        type=int,
        default=64,
        help="Number of hidden units in policy/value networks",
    )
    parser.add_argument(
        "--layers",
        type=int,
        default=2,
        help="Number of hidden layers in policy/value networks",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of parallel CPUs to use (requires MPI)",
    )
    ...
    parser.add_argument(
        "--exp_name", type=str, default="ppo_mapgen", help="Experiment name for logging"
    )
    parser.add_argument(
        "--render_freq", type=int, default=10, help="Render map every N episodes"
    )
    ...
    logger_kwargs = dict(
        output_dir=f"data/{args.exp_name}_s{args.seed}", exp_name=args.exp_name
    )
    ...
    env_fn = lambda: MapGenEnv(
        map_size=MAP_SIZE,
        hero_param_ranges=HERO_PARAM_RANGES,
        max_steps=args.max_steps,
        render_mode="human" if proc_id() == 0 else None,
    )  # Only render on proc 0
